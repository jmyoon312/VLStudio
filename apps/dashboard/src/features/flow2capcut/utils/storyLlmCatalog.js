...
export function normalizeStoryLlmOptions(options = {}, catalog = STORY_LLM_OPTIONS) {
  ...
  if (searchOptions.engine === 'codex') {
    searchOptions = { engine: 'claude', model: 'claude-opus-4-8' }
    if (!findStoryLlmOptionById('claude:claude-opus-4-8', catalog) && catalog[0]) {
       searchOptions = { engine: catalog[0].engine, model: catalog[0].model }
    }
  }
  if (searchOptions.engine && searchOptions.model && !findStoryLlmOptionForOptions(searchOptions, catalog)) {
    throw new Error(`Unknown Story LLM option: ${searchOptions.engine}:${searchOptions.model}`)
  }
  const selectionId = hydrateStoryLlmSelection(searchOptions, catalog)
  const selected = findStoryLlmOptionById(selectionId, catalog) || catalog[0] || DEFAULT_STORY_LLM
  if (!searchOptions.engine && typeof searchOptions.model === 'string' && searchOptions.model.startsWith('gemini-')) {
    const normalized = stripRuntimeControlOptions(searchOptions)
    delete normalized.llmId
    delete normalized.id
    return normalized
  }
  const normalized = { ...stripRuntimeControlOptions(searchOptions), engine: selected.engine, model: selected.model }
  ...
  if (allowed.length) {
    normalized.reasoningEffort = allowed.includes(searchOptions.reasoningEffort)
      ? searchOptions.reasoningEffort
