...
function App() {
  ...
  // 일반 생성 화면의 프리뷰(LiveTimeline) 및 내보내기(Export)는 메인 audioPackage에 더해
  // story 세그먼트 오디오(화자별 voices, sfx)를 얹어 타임라인 및 내보내기에 온전히 반영한다.
  const effectiveAudioPackage = useMemo(() => {
    let pkg = withStoryAudio(audioPackage, storyPipeline.scenes || [])
    const hasVoices = pkg?.voices?.some((v) => v.files.length > 0)
    const hasSfx = pkg?.sfx?.some((s) => s.files.length > 0)
    if (!hasVoices && !hasSfx && scenes?.length > 0) {
      pkg = withStoryAudio(pkg, scenes)
    }
    return pkg
  }, [audioPackage, storyPipeline.scenes, scenes])
  ...
  useMcpServer({
    ...
    importByPath, audioPackage: effectiveAudioPackage,
          ...
          <>
            ...
            {bottomPanelView === 'timeline' ? (
              <LiveTimeline
                ...
                onSceneUpdate={scenesHook.updateScene}
                disabled={anyRunning}
                onTitleClick={() => setMonitorOverlayOpen(o => !o)}
