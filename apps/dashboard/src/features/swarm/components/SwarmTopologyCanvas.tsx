...
const initialEdges: Edge[] = [
    { id: 'e_g_c', source: 'global_overseer', target: 'coordinator_1', animated: true, style: { stroke: '#6366f1', strokeWidth: 3 } },
    ...
    { id: 'e_w_e', source: 'writer_1', target: 'editor_1', animated: false, style: { stroke: '#cbd5e1', strokeWidth: 2 } },
    { id: 'e_m_e', source: 'media_1', target: 'editor_1', animated: false, style: { stroke: '#cbd5e1', strokeWidth: 2 } },
    { id: 'e_e_p', source: 'editor_1', target: 'publisher_1', animated: false, style: { stroke: '#cbd5e1', strokeWidth: 2 } },
    { id: 'e_p_a', source: 'publisher_1', target: 'analyst_1', animated: false, style: { stroke: '#cbd5e1', strokeWidth: 2 }, label: '지연 동기화' },
...
export const SwarmTopologyCanvas: React.FC<SwarmTopologyCanvasProps> = ({ onNodeClick }) => {
    ...
    return (
        <div className="w-full h-full bg-slate-50 rounded-[3rem] overflow-hidden border border-slate-200 shadow-inner relative">
            <ReactFlow
                nodes={nodes}
                edges={edges}
            ...
            >
                <Background color="#cbd5e1" gap={24} size={1} className="opacity-20" />
                <Controls className="bg-white border-slate-200 shadow-2xl rounded-2xl overflow-hidden" />
            </ReactFlow>
            
            {/* HUD Overlay */}
            <div className="absolute top-12 left-12 pointer-events-none">
                <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 p-6 rounded-[2rem] shadow-2xl">
                    <h2 className="text-white font-black uppercase tracking-[0.2em] text-xs italic">실시간 작업 흐름도</h2>
                    <p className="text-slate-400 text-[10px] font-black tracking-widest mt-2 uppercase opacity-60">Autonomous Graph • Model: Directed Acyclic</p>
                </div>
            </div>
            
            <div className="absolute bottom-8 left-8 pointer-events-none flex gap-4">
                <div className="bg-indigo-600 shadow-xl shadow-indigo-100 text-white font-black text-[10px] px-6 py-3 rounded-full uppercase tracking-widest flex items-center gap-3">
                    <div className="w-2 h-2 bg-white rounded-full animate-pulse shadow-[0_0_10px_white]" />
                    함대 노드 활성 (ACTIVE)
                </div>
                {isConnected ? (
                    <div className="bg-emerald-500 shadow-xl shadow-emerald-100 text-white font-black text-[10px] px-6 py-3 rounded-full uppercase tracking-widest flex items-center gap-3">
                        <div className="w-2 h-2 bg-white rounded-full animate-pulse shadow-[0_0_10px_white]" />
                        소켓 동기화 (ONLINE)
                    </div>
                ) : (
                    <div className="bg-rose-500 shadow-xl shadow-rose-100 text-white font-black text-[10px] px-6 py-3 rounded-full uppercase tracking-widest flex items-center gap-3">
