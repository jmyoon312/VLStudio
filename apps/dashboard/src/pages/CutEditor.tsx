...
const CutEditor = () => {
    ...
    return (
        <div className="h-screen flex flex-col bg-white overflow-hidden text-slate-900 font-sans selection:bg-blue-100">
            <header className="h-12 border-b border-slate-200 bg-white flex items-center px-4 justify-between shadow-sm z-50 shrink-0">
                <div className="flex items-center gap-4">
                    <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold">SA</div>
                    <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
                        <Button variant={canvasMode === 'shorts' ? 'default' : 'ghost'} size="sm" className={cn("h-7 px-3 text-xs", canvasMode === 'shorts' && "bg-blue-600 text-white")} onClick={() => { setCanvasMode('shorts'); setAspectRatio('9:16'); }}>Shorts</Button>
                        <Button variant={canvasMode === 'wide' ? 'default' : 'ghost'} size="sm" className={cn("h-7 px-3 text-xs", canvasMode === 'wide' && "bg-blue-600 text-white")} onClick={() => { setCanvasMode('wide'); setAspectRatio('16:9'); }}>Wide</Button>
                    </div>
                </div>
                <div className="flex items-center gap-2"><Button variant="outline" size="sm" className="h-8" onClick={() => setIsExportModalOpen(true)}>Export</Button><Button size="sm" className="h-8 bg-blue-600">Save</Button></div>
            </header>

            <div className="flex-1 flex overflow-hidden relative">
                <div className="flex h-full border-r border-slate-200 bg-white z-30 shadow-sm shrink-0">
                    <nav className="w-16 flex flex-col items-center py-4 gap-2 bg-slate-50/50 border-r">{sidebarItems.map(i => <button key={i.id} onClick={() => setActiveSidebarTab(i.id)} className={cn("w-12 p-2 rounded flex flex-col items-center gap-1", activeSidebarTab === i.id ? "text-blue-600 bg-blue-50" : "text-slate-500")}><i.icon className="w-5 h-5" /><span className="text-[10px]">{i.label}</span></button>)}</nav>
                    <aside className="w-64 flex flex-col"><div className="h-12 border-b px-4 flex items-center font-semibold text-sm">{sidebarItems.find(i => i.id === activeSidebarTab)?.label}</div><div className="flex-1 overflow-auto">
                ...
                <main className="flex-1 overflow-hidden flex flex-col items-center justify-center bg-slate-100 relative min-w-0">
                    <div className="absolute top-6 z-50 flex gap-2 bg-white/90 backdrop-blur px-4 py-1 rounded-full border shadow-sm">
                        {selectedClipId ? <><Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setIsMotionOpen(true)}><Wand2 className="w-3.5 h-3.5 mr-1" />효과</Button>
                            ...
                            <div className="w-px h-4 bg-slate-300" />
                            <Button size="sm" variant="ghost" className="h-7 text-xs text-red-500" onClick={() => { const t = tracks.find(t => t.clips.some(c => c.id === selectedClipId)); if (t) removeClip(t.id, selectedClipId); }}><Trash2 className="w-3.5 h-3.5 mr-1" />삭제</Button></> : <span className="text-xs text-slate-400">클립을 선택하세요</span>}
                    </div>
                    {/* Center Canvas Area */}
                    ...
                    <div className="absolute bottom-6 z-50 flex flex-col items-center gap-3">
                        {/* Zoom Controls */}
                        <div className="flex items-center gap-2 bg-black/80 backdrop-blur px-3 py-1.5 rounded-full shadow-xl border border-white/10 text-white">
                            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full hover:bg-white/20 hover:text-white text-white/70" onClick={() => setPreviewScale(Math.max(0.1, previewScale - 0.1))}><ZoomOut className="w-3.5 h-3.5" /></Button>
                            ...
                            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full hover:bg-white/20 hover:text-white text-white/70" onClick={() => setPreviewScale(Math.min(4, previewScale + 0.1))}><ZoomIn className="w-3.5 h-3.5" /></Button>
                            <div className="w-px h-3 bg-white/20 mx-1" />
                            <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px] hover:bg-white/20 hover:text-white text-white/70 rounded-full" onClick={() => setPreviewScale(1)}>Fit</Button>
                            <span className="text-[10px] font-mono w-8 text-center">{Math.round(previewScale * 100)}%</span>
                        </div>

                        {/* Playback Controls */}
                        <div className="flex gap-4 bg-white/90 backdrop-blur px-5 py-2 rounded-full shadow-xl border"><Button variant="ghost" size="icon" className="h-9 w-9 rounded-full" onClick={() => setIsPlaying(!isPlaying)}>{isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 ml-0.5" />}</Button></div>
                    </div>
                </main>

                <aside className="w-[380px] border-l bg-white h-full overflow-y-auto z-20 shrink-0 shadow-lg">
                    <div className="h-12 border-b px-4 flex items-center font-semibold text-sm">Properties</div>
                    {selectedClipId ? <PropertyPanel /> : <div className="p-10 text-center text-slate-400 text-sm">Select clip</div>}
                </aside>
            </div >


            <footer className="h-[35vh] border-t bg-white flex flex-col z-50 shadow-lg shrink-0">
                <div className="h-10 border-b px-4 flex items-center justify-between">
                    <div className="flex gap-1"><Button variant="ghost" size="sm" className="w-7 h-7 p-0" onClick={() => useEditorStore.temporal.getState().undo()}><Undo className="w-4 h-4" /></Button><Button variant="ghost" size="sm" className="w-7 h-7 p-0" onClick={() => useEditorStore.temporal.getState().redo()}><Redo className="w-4 h-4" /></Button></div>
                    <div className="flex gap-2"><Button variant="ghost" size="sm" className="h-7 text-xs text-red-500" onClick={() => resetEditor()}><RotateCcw className="w-3.5 h-3.5 mr-1" />초기화</Button><div className="w-px h-4 bg-slate-200" /><Button variant={activeTool === 'select' ? 'secondary' : 'ghost'} size="sm" className="h-7 text-xs" onClick={() => setActiveTool('select')}><MousePointer2 className="w-3.5 h-3.5 mr-1" />선택</Button><Button variant={activeTool === 'split' ? 'secondary' : 'ghost'} size="sm" className="h-7 text-xs" onClick={() => setActiveTool('split')}><Scissors className="w-3.5 h-3.5 mr-1" />분할</Button><Button variant="ghost" size="sm" className="h-7 text-xs text-blue-600" onClick={handleAutoCut} disabled={isDetectingScenes}><Wand2 className="w-3.5 h-3.5 mr-1" />AI 자르기</Button></div>
                    <div className="flex gap-3"><Button variant={tracks.find(t => t.id === 'main-video')?.isMagnetic ? 'secondary' : 'ghost'} size="sm" className="w-7 h-7 p-0" onClick={() => toggleMagnetic('main-video')}><Magnet className="w-4 h-4" /></Button><div className="w-px h-4 bg-slate-200" /><Slider value={[scale]} min={10} max={200} step={10} onValueChange={v => setScale(v[0])} className="w-28" /></div>
                </div>
                <div className="flex-1 flex overflow-hidden bg-slate-50/50">
                    <div ref={headerRef} className="w-[173px] shrink-0 bg-white border-r flex flex-col" onWheel={handleHeaderWheel}><div className="h-8 border-b bg-slate-50 shrink-0" /><div className="p-2 space-y-2">{tracks.map((t, i) => <div key={t.id} className="h-16 flex items-center px-2 text-xs bg-slate-50 border rounded-lg"><div className="flex-1 min-w-0 flex gap-2 items-center">{t.type === 'video' ? <Video className="w-3.5 h-3.5 text-blue-500" /> : t.type === 'audio' ? <Volume2 className="w-3.5 h-3.5 text-emerald-500" /> : t.type === 'image' ? <ImageIcon className="w-3.5 h-3.5 text-orange-500" /> : <Type className="w-3.5 h-3.5 text-purple-500" />}<span className="truncate">{t.label}</span></div><div className="flex gap-1">{(t.type === 'video' || t.type === 'audio') && <button onClick={e => { e.stopPropagation(); toggleTrackMute(t.id) }} className="p-1">{t.muted ? <VolumeX className="w-3 h-3 text-red-500" /> : <Volume2 className="w-3 h-3" />}</button>}<button onClick={e => { e.stopPropagation(); toggleTrackHide(t.id) }} className="p-1">{t.hidden ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}</button><button onClick={e => { e.stopPropagation(); addTrack(t.type, i) }} className="p-1"><Plus className="w-3 h-3" /></button>{!t.isDefault && <button onClick={e => { e.stopPropagation(); removeTrack(t.id) }} className="p-1 text-red-500"><Trash2 className="w-3 h-3" /></button>}</div></div>)}</div></div>
                    <div ref={timelineRef} className="flex-1 overflow-auto relative" onScroll={handleScroll} onClick={handleTimelineClick} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const d = e.dataTransfer.getData('application/json'); if (d) addClip(null, null, JSON.parse(d).source, JSON.parse(d).type, Math.max(0, (e.clientX - timelineRef.current!.getBoundingClientRect().left + timelineRef.current!.scrollLeft) / scale)) }}>
                        <div className="relative min-h-full" style={{ width: duration * scale + 500 }}>
                            <div className="h-8 border-b bg-white sticky top-0 z-20 flex items-end">{Array.from({ length: Math.ceil(duration) + 5 }).map((_, i) => <div key={i} className="border-l h-3 relative" style={{ width: scale }}><span className="absolute -top-4 left-1 text-[10px] text-slate-400">{i}s</span></div>)}</div>
                            <div className="absolute top-8 bottom-0 left-0 right-0 pointer-events-none">{Array.from({ length: Math.ceil(duration) + 5 }).map((_, i) => <div key={i} className="absolute top-0 bottom-0 border-l border-slate-100" style={{ left: i * scale }} />)}</div>
                            <div className="absolute top-0 bottom-0 w-px bg-red-500 z-50 pointer-events-none" style={{ transform: `translateX(${currentTime * scale}px)` }}><div className="absolute -top-1 -left-1.5 w-3 h-3 bg-red-500 rotate-45" /><div className="absolute top-0 bottom-0 w-px bg-red-500/50" /></div>
                            <div className="py-2 space-y-2 relative z-10">{tracks.map(t => <div key={t.id} data-track-id={t.id} className="h-16 relative bg-slate-100/30 rounded-lg border">{t.clips.map(c => <ClipItem key={c.id} clip={c} track={t} scale={scale} selectedClipId={selectedClipId} onDrag={(tr: string, cl: string, d: any, tg: any) => moveClip(tr, cl, Math.max(0, d.x / scale), tg)} onSelect={setSelectedClipId} onResize={resizeClip} />)}</div>)}</div>
                        </div>
                    </div>
                </div>
            </footer>
...
const ClipItem: React.FC<any> = ({ clip, track, scale, selectedClipId, onDrag, onSelect, onResize }) => {
    ...
    return <Draggable nodeRef={nodeRef} grid={[1, 1]} position={{ x: clip.start * scale, y: 0 }} onStop={(e, d) => { const m = e as MouseEvent; const tg = document.elementsFromPoint(m.clientX, m.clientY).find(el => el.hasAttribute('data-track-id'))?.getAttribute('data-track-id'); onDrag(track.id, clip.id, d, tg) }} disabled={track.locked}>
        <div ref={nodeRef} className={cn("absolute top-1 bottom-1 rounded border cursor-pointer", selectedClipId === clip.id ? "ring-2 ring-blue-500 z-20" : "z-10", clip.type === 'video' ? "bg-blue-100 border-blue-300" : clip.type === 'audio' ? "bg-emerald-100 border-emerald-300" : "bg-purple-100 border-purple-300")} style={{ width: clip.duration * scale }} onClick={e => { e.stopPropagation(); onSelect(clip.id) }}>
