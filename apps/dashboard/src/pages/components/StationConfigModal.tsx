        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-0 overflow-hidden flex flex-col max-h-[85vh]">
                <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900">방송 송출 설정 (Broadcast Config)</h2>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className={`p-3 rounded-xl border text-left transition-all ${serverMode === 'local' ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200' : 'border-gray-200 hover:bg-gray-50'}`}
                            >
                                <div className="text-sm font-bold text-gray-900">로컬 서버 (Local)</div>
                                <div className="text-xs text-gray-500 mt-1">현재 PC에서 직접 송출합니다.</div>
                            </button>
                            <button
                                onClick={() => setServerMode('external')}
                                className={`p-3 rounded-xl border text-left transition-all ${serverMode === 'external' ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200' : 'border-gray-200 hover:bg-gray-50'}`}
                            >
                                <div className="text-sm font-bold text-gray-900">외부 서버 (Relay)</div>
                                <div className="text-xs text-gray-500 mt-1">외부 릴레이/클라우드 서버 사용</div>
                            </button>
                        </div>
                    </div>
                        ...
                        {serverMode === 'local' ? (
                            ...
                            <>
                                <div className="space-y-2">
                                    ...
                                    >
                                        <option value="">-- 채널 선택 (YT 자동 설정) --</option>
                                        {channels.map(c => (
                                ...
                                <div>
                                    <label className="text-xs text-gray-500 font-bold mb-1 block">RTMP URL (수동 입력)</label>
                                    <input
                                        value={rtmpUrl}
                            ...
                            <div className="space-y-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                                <div className="flex items-center justify-between">
                                    <h4 className="text-xs font-bold text-gray-500 uppercase">Server Config</h4>
                                    <button
                                        onClick={() => setShowGuide(true)}
                                        className="text-xs bg-white border border-gray-300 px-2 py-1 rounded shadow-sm hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 font-bold flex items-center gap-1 transition-colors"
                                    >
                                ...
                                <div className="grid grid-cols-[2fr_1fr] gap-3">
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">서버 주소 (IP/Domain)</label>
                                        <input
                                            value={extHost}
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">포트 (Port)</label>
                                        <input
                                            value={extPort}
                                ...
                                <div className="grid grid-cols-[1fr_2fr] gap-3">
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">App Name</label>
                                        <input
                                            value={extApp}
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">Stream Key</label>
                                        <input
                                            value={extKey}
                                ...
                                <div className="pt-2 border-t border-gray-200">
                                    <label className="text-xs font-bold text-gray-500 block mb-1">최종 RTMP URL (자동 생성)</label>
                                    <div className="w-full bg-gray-100 border border-gray-200 rounded px-3 py-2 font-mono text-xs text-gray-600 break-all">
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 shrink-0">
                    ...
                    <button
                        onClick={handleSave}
                        className="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold shadow-lg shadow-blue-200 transition-all hover:translate-y-px"
                    >
                        설정 저장
                    </button>
                </div>
