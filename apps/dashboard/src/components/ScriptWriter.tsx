...
const ScriptWriter = () => {
    ...
    // [NEW] 50초 숏폼 및 롱폼 듀얼 트랙 각색 핸들러 (DB Settings 실시간 동적 연동)
    const [isDualAdapting, setIsDualAdapting] = useState(false);
    const [activeTrackMode, setActiveTrackMode] = useState<'shorts' | 'longform' | null>(() => {
        return (location.state as any)?.track || null;
    });

    const handleDualTrackAdapt = async (track: 'shorts' | 'longform') => {
        if (!inputText.trim()) {
            toast.error("원본 대본 또는 텍스트를 입력해주세요.");
            return;
        }
        setIsDualAdapting(true);
        setActiveTrackMode(track);
        try {
            const res = await api.post('/creative-scripts/dual-track-adapt', {
                source_script: inputText,
                track: track,
                style_tone: track === 'shorts' ? '바이럴 흥미 유발' : '고품격 스토리텔링 다큐'
            });
            if (res.data?.script) {
                pushHistory(resultText);
                setResultText(res.data.script);
                toast.success(
                    track === 'shorts'
                        ? "⚡ 50초 숏폼 3단 압축 각색 완료!"
                        : "🎬 100% 오리지널 전면 롱폼 재창작 완료!"
                );
            }
        } catch (e: any) {
            toast.error("각색 실패: " + (e.response?.data?.detail || e.message));
        } finally {
            setIsDualAdapting(false);
        }
    };
    ...
    useEffect(() => {
        ...
        // 2. URL Query Params (?video_id=...&topic=...&script=...) 체크
        ...
        if (videoId) {
            ...
            api.get(`/videos/${videoId}`)
                ...
                .then(res => {
                    ...
                    const loadedText = videoData.content || videoData.extracted_text || videoData.description || videoData.title || (topic ? decodeURIComponent(topic) : "");

                    if (loadedText) {

                        setInputText(loadedText);

                        toast.success(`'${videoData.title || '선택한 영상'}' 대본을 불러왔습니다.`);
                ...
                .catch(() => {
                    ...
                    if (topic) setInputText(decodeURIComponent(topic));

                });

        } else if (script) {

            setInputText(decodeURIComponent(script));

            toast.success("대본을 불러왔습니다.");

        } else if (topic) {

            setInputText(decodeURIComponent(topic));

            toast.success(`'${decodeURIComponent(topic)}' 주제를 불러왔습니다.`);
                    ...
                    <CardHeader className="py-2.5 sm:py-3 px-3 sm:px-4 border-b border-border bg-muted/30">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-1.5 sm:gap-2">
                                ...
                                {/* 차세대 파이프라인 즉시 내보내기 버튼 */}
                                <Button
                                    size="sm"
                                    onClick={() => {
                                        navigate('/ddalkkak', {
                                            state: {
                                                scriptText: resultText,
                                                initialTab: 'subtitle'
                                            }
                                        });
                                        toast.success("AI 쇼츠 딸깍 제작소로 대본을 전송했습니다!");
                                    }}
                                    className="h-7 sm:h-8 px-2.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-xs flex items-center gap-1.5 active:scale-95"
                                    title="50초 대본으로 AI 쇼츠 영상 자동 제작"
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        navigate('/creative-studio', {
                                            state: {
                                                referenceScript: resultText,
                                                track: 'longform'
                                            }
                                        });
                                        toast.success("Google Flow 롱폼 스튜디오로 대본을 전송했습니다!");
                                    }}
                                    className="h-7 sm:h-8 px-2.5 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs flex items-center gap-1.5 active:scale-95"
                                    title="롱폼 대본으로 Google Flow AI 씬 분할 및 미디어 생성"
                                >
                                    <Sparkles className="w-3 h-3 text-emerald-200" />
                                    <span>🎬 롱폼 AI 스튜디오</span>
                                </Button>
