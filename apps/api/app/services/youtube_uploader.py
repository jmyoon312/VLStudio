        ...
        try:
            ...
            if not tin_can.proxy_config:
                ...
                if tin_can.last_upload_ip and tin_can.last_upload_ip == current_ip:
                    ...
                    adb_service.toggle_mobile_data()
                    import time
                    time.sleep(5) 
                    current_ip = adb_service.get_current_ip()
                    
                    if current_ip == tin_can.last_upload_ip:
                        logger.warning("Soft Rotation Failed. Escalating to Hard Rotation...")
                        # 2. Try Hard Rotation (Airplane Mode)
                        adb_service.toggle_airplane_mode()
                        time.sleep(5)
                        current_ip = adb_service.get_current_ip()
