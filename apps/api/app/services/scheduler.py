...
def update_video_stats():
    ...
    try:
        ...
        # Fetch candidate IDs
        all_videos = db.query(models.Video.id, models.Video.url, models.Video.title).all()
        if not all_videos:
            return
            
        # Shuffle and pick batch (increased slightly but with delays)
        target_batch = random.sample(all_videos, k=min(len(all_videos), 20))
