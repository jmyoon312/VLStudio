...
class CapCutRegistryManager:
    ...
    def __init__(self):
        ...
        self.root_path = os.path.join(os.environ.get("LOCALAPPDATA", "C:"), r"CapCut\User Data\Projects\com.lveditor.draft")
        self.meta_file = os.path.join(self.root_path, "root_meta_info.json")

    def register_project(self, project_name, folder_name, draft_id, duration_ms, exact_folder_path=None):
        """
        ...
        # 드라이브 문자는 대문자 'C:' 선호
        if exact_folder_path:
            fold_path = os.path.normpath(exact_folder_path).replace("\\", "/")
            project_root_path = os.path.dirname(os.path.normpath(exact_folder_path)).replace("\\", "/")
        else:
            fold_path = os.path.join(self.root_path, folder_name).replace("\\", "/")
            project_root_path = self.root_path.replace("\\", "/")
            
        if fold_path[0].islower():
            fold_path = fold_path[0].upper() + fold_path[1:]
        if project_root_path[0].islower():
            project_root_path = project_root_path[0].upper() + project_root_path[1:]
        ...
        new_entry = {
            ...
            "draft_new_version": "",
            "draft_root_path": project_root_path,
