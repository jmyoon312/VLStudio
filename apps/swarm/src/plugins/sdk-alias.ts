...
function listPluginSdkSubpathsFromPackageJson(pkg: PluginSdkPackageJson, packageRoot?: string): string[] {
  const subpaths = Object.keys(pkg.exports ?? {})
    .filter((key) => key.startsWith("./plugin-sdk/"))
    .map((key) => key.slice("./plugin-sdk/".length))
    .filter((subpath) => isSafePluginSdkSubpathSegment(subpath));

  if (subpaths.length === 0 && packageRoot) {
    try {
      const sdkDir = path.join(packageRoot, "src", "plugin-sdk");
      const entries = fs.readdirSync(sdkDir);
      for (const entry of entries) {
        if (entry.endsWith(".ts")) {
          const name = entry.slice(0, -3);
          if (isSafePluginSdkSubpathSegment(name)) {
            subpaths.push(name);
          }
        }
      }
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  const subpaths = listPluginSdkSubpathsFromPackageJson(pkg, packageRoot);
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  console.log(`[sdk-alias] Resolved package root: ${root} from ${params.modulePath}`);
  return root;
}
...
export function buildPluginLoaderAliasMap(
  ...
  const pluginSdkAlias = resolvePluginSdkAliasFile({
    srcFile: "", // Empty means use the directory
