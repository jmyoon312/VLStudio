...
if (
  ...
  */
  ...
  // [HERMES DYNAMIC OVERRIDE]
  if (process.env.AGENT_MODEL) {
    console.error(`[HERMES] Injecting Dynamic Model: ${process.env.AGENT_MODEL}`);
    setConfigOverride("agents.defaults.model", process.env.AGENT_MODEL);
  }
  
  // [HERMES THINKING SUPPRESSION]
  // Force disable thinking indicators globally if requested or by default for Hermes
  console.error(`[HERMES] Suppressing Thinking Signals for direct response.`);
  setConfigOverride("agents.defaults.thinkingDefault", false);
  setConfigOverride("agents.defaults.verboseDefault", false);
