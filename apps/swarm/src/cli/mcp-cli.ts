...
import { serveOpenClawLimbMcp } from "../mcp/limb-server.js";
...
export function registerMcpCli(program: Command) {
  ...
  mcp
    .command("serve-limb")
    .description("Expose OpenClaw Agent and Production tools over MCP stdio (Native)")
    .action(async () => {
      try {
        await serveOpenClawLimbMcp();
      } catch (err) {
        defaultRuntime.error(String(err));
        defaultRuntime.exit(1);
      }
