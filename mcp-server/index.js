...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    // ── 프로젝트 관리 도구 ──
    {
      ...
      description: 'ViraLoop Studio 작업 폴더의 프로젝트 목록을 조회합니다.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'app_status',
      description: 'ViraLoop Studio 앱의 HTTP 서버 상태를 확인합니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 현재 레퍼런스 목록을 가져옵니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱의 레퍼런스를 직접 수정합니다. (project.json 우회, 앱 상태 직접 변경)',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 레퍼런스 카드를 삭제합니다. (UI의 "카드 제거" 버튼과 동일)',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 레퍼런스의 이미지만 제거합니다. (카드는 유지, UI의 "이미지만 제거" 버튼과 동일)',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 모든 레퍼런스의 이미지를 일괄 제거합니다. (카드는 유지, 프롬프트/이름은 보존)',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    {
      name: 'app_get_scenes',
      description: '실행 중인 ViraLoop Studio 앱에서 현재 씬 목록을 가져옵니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱의 특정 씬을 직접 수정합니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 레퍼런스 이미지를 생성합니다. (프롬프트 기반 Flow API 이미지 생성 트리거). styleId를 지정하면 해당 스타일을 적용합니다. 이미 이미지가 있는 reference라도 다시 호출하면 새 styleId로 재생성됩니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 특정 씬의 이미지를 생성합니다. styleId를 지정하면 해당 스타일로 강제 적용됩니다. 이미 이미지가 있는 씬이라도 다시 호출하면 새 styleId로 재생성됩니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 씬 일괄 생성(생성 시작 버튼)을 트리거합니다. pending 상태인 씬들의 이미지를 자동 생성합니다. force=true면 완료된 씬도 포함해 강제 재생성합니다. 진행 중에 호출하면 자동으로 중지 후 새 스타일로 재시작합니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
    ...
    },
    {
      ...
      description: '실행 중인 ViraLoop Studio 앱에서 레퍼런스 일괄 생성을 트리거합니다. 모든 레퍼런스의 이미지를 자동 생성합니다. force=true면 완료된 reference도 포함해 강제 재생성합니다. 진행 중에 호출하면 자동으로 중지 후 새 스타일로 재시작합니다.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'number', description: 'HTTP 서버 포트 (기본: 3210)' },
...
server.setRequestHandler(ListPromptsRequestSchema, async () => ({
  prompts: [
    {
      name: 'setup',
      description: 'ViraLoop Studio 스킬 설치 상태를 확인하고 미설치 스킬을 안내합니다. MCP 연동 후 처음 사용할 때 실행하세요.',
    },
  ],
}));
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  if (name === 'setup') {
    ...
    let message = '# ViraLoop Studio 스킬 설정\n\n';
...
async function main() {
  ...
  console.error('ViraLoop Studio MCP Server 시작됨 (stdio)');
}
