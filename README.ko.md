...
* **수집 영상 보관함 (`/gallery`)**:
  * 1080p Full HD 무손실 표준 저장 및 바이럴 지수(Velocity Score)·등급(S/A/B)별 정렬.
  * **`[🔗 유튜브 원본]` 바로가기 원클릭 지원** 및 **`⚡ AI 제작 가능`** 상태 뱃지.
  * 하이브리드 폴더 열기(PC 탐색기 열기 / 웹·모바일 클립보드 자동 복사 지원).
* **수집 대본 분석실 (`/script-lab`)**: Whisper AI 음성 자막 추출, 3초 후킹/본문/CTA 구간 분할 및 유튜브 원본 링크 대조 분석.
...
* **쇼츠 자동 배포 관리 (`/work-queue`)**: 
  * **실시간 다차원 통합 검색**: 제목, 채널명, 설명, 파일명, ID, 프로젝트명을 실시간 검색 + 빠른 지우기(X).
  * **정밀 다채널 & 업로드 방식 필터**: 유튜브/틱톡/인스타 채널별 필터, 스텔스 자동/수동/즉시 등록 방식별 필터.
  * **다기능 정렬 & 토글**: 등록순, 예약순, 채널순, 상태순 정렬 + 오름/내림차순 원클릭 전환.
  * **모바일 2열 가로 스크롤 최적화**: 화면 크기에 구애받지 않는 터치 친화적 칩 바 레이아웃.
  * **픽셀링(Pixeling) 메타 자동 파싱 및 1:1 표준 네이밍 매핑**.
* **채널 계정 & 웜업 육성 (`/incubator`)**: 
  * **일괄 웜업 육성 제어 KPI 카드**: 미니멀 카드 스타일의 6대 핵심 지표 관제.
  * **모바일 프리미엄 계정 카드**: 이메일 풀 노출, 채널명 인코딩 정제, 스택형 웜업 제어 바 탑재.
  * LTE 동적 IP 및 1:1 ISP 고정 IP 하이브리드 바인딩 + 7단계 인간 행동 모사 웜업.
