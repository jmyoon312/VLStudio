...
export class PyTaskRunnerProcess extends TaskRunnerProcessBase {
	...
	static async checkRequirements(): Promise<'python' | 'venv' | null> {
		// [PATCH] Skip checks if external python is provided
		if (process.env.EXT_PYTHON_PATH) {
			try {
				// Verify the external python works
				await asyncExec(`"${process.env.EXT_PYTHON_PATH}" --version`, { timeout: 5000 });
				return null;
			} catch {
				return 'python';
			}
	...
	private static getVenvPath() {
		// [PATCH] Support external python path from env
		if (process.env.EXT_PYTHON_PATH) {
			return process.env.EXT_PYTHON_PATH;
		}
		const pythonDir = path.join(__dirname, '../../../@n8n/task-runner-python');
