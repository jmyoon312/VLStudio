...
- `{PROJECT_DIR}/ep{number}_{slug}/_story_source/` — episode directory (inside ViraLoop Studio work folder, never inside source repo)
- `STATE.md` — workflow state (current wave, decisions)
- `W_progress.json` — progress log for external tools

**{PROJECT_DIR} resolution**: Call `mcp__viraloop__app_list_projects` — the `작업폴더:` line in its output is the work folder. Never fall back to cwd or to the source-code repo.
