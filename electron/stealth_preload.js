...
(function() {
  ...
  try {
    ...
    // Override webdriver flag (runs on all domains excluding accounts.google.com)
    Object.defineProperty(navigator, 'webdriver', {
      get: makeNativeGetter(false, 'Navigator.webdriver'),
      configurable: true
    });

    // Override userAgentData to mask "Electron" and mimic real Chrome (runs on all domains excluding accounts.google.com)
    if (navigator.userAgentData) {
      const mockUserAgentData = {
        ...
        mobile: false,
        platform: 'Windows',
        getHighEntropyValues: makeNativeGetter(
          function(hints) {
      ...
      Object.defineProperty(navigator, 'userAgentData', {
        get: makeNativeGetter(mockUserAgentData, 'Navigator.userAgentData'),
        configurable: true
      });
    }
  } catch (e) {
    console.warn('[Stealth Preload] Webdriver/userAgentData override failed:', e.message);
  }

  try {
    const makeNativeGetter = globalThis.__makeNativeGetter;

    // [Moved WebAuthn bypass to top of IIFE]
  ...
  try {
    ...
    console.error('[Stealth Preload] Failed to expose electronAPI:', e.message)
  }
})();
