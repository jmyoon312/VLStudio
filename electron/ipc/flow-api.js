...
export function registerFlowAPIIPC(ipcMain, deps) {
  ...
  ipcMain.handle('flow:generate-image', async (event, {
    ...
    try {
      ...
      // 0. Flow 프로젝트 페이지 및 에디터 확인/진입 (ensureOnProjectComposer)
      const projectCheck = await ensureOnProjectComposer(flowView, projectId)
      if (!projectCheck.ok) {
        return { success: false, error: projectCheck.error || 'Flow project not ready' }
      }
      ...
      // 0.8. 이미지 모드 + 배치 + 화면비 설정 (Settings에서 전달받은 값 사용, 기본 x1)
      // aspectRatio 는 설정 메뉴가 열려 있는 동안 함께 적용된다 — 메뉴를 닫은 뒤
      // 별도로 동기화하면 Radix 가 content 를 unmount 해서 동작하지 않기 때문.
      const effectiveBatchCount = Math.max(1, Math.min(4, Number(batchCount) || 1))
